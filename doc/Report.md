...
L'applicativo consente di giocare una partita di scacchi accettando in input mosse in notazione algebrica abbreviata italiana (vedi paragrafo  **7. Manuale Utente**).
...
### a. Requisiti funzionali
 ...
 - **Come utente, voglio vedere la scacchiera**:
	...
	Al comando `board` mostra la scacchiera completa di coordinate e pezzi in formato Unicode;
 ...
 - **Come utente, voglio visualizzare le catture**:
	...
	Al comando `captures`, si mostra il materiale vinto da entrambi i colori in formato Unicode;
 ...
 - **Come utente, voglio arroccare corto**:
	 ...
	 Attraverso una mossa in notazione algebrica abbreviata, devo poter arroccare corto secondo le regole degli scacchi ( come da esempio nel paragrafo **7. g. Mosse illegali**);
 - **Come utente, voglio arroccare lungo**:
 
	 Attraverso una mossa in notazione algebrica abbreviata, devo poter arroccare lungo secondo le regole degli scacchi ( come da esempio nel paragrafo **7. g. Mosse illegali**);
...
- La suddivisione delle classi, determinata da accorpamenti di responsabilità comuni, avviene in 3 package:

	- `it.uniba.chess` : contiene tutti gli elementi relativi alla logica del gioco degli scacchi
	- `it.uniba.chess.pieces` : contiene tutte le classi specializzate dei pezzi e la loro classe astratta Piece
	- `it.uniba.chess.utils` : contiene classi con comportamenti utili all'interno del programma

- L'input dell'utente viene analizzato e sanificato dalla funzione `parseCommand()` della classe `InputValidator` in modo da avere un entry-point comune e rigettare quanto prima possibile un comando non riconosciuto o una mossa illegale senza interagire con lo stato della scacchiera o della partita;
...
- Le mosse (movimenti e catture) non sono state inserite nei singoli pezzi per evitare dipendenze dal resto della scacchiera o dallo stato del gioco. I pezzi devono esistere come elementi a sè stanti, anche al di fuori della scacchiera;
...
- Le tre classi Singleton sono state scelte come tali per ottimizzare i tempi di esecuzione e snellire il codice, vista l'impossibilità di avviare contemporaneamente più di una partita.

- `InputValidator` e `Move` , tra loro complementari, vengono separate per avere un ruolo specifico: la prima si occupa di lavorare con stringhe e riconoscere la mossa, la seconda è quella che ne calcola l'effettiva validità secondo le regole degli scacchi basandosi sullo stato della scacchiera e, in caso positivo, modifica lo stato della scacchiera stessa e del gioco.
...
- ### e. Mosse supportate e relativa notazione
	 ...
	 Generalmente, per muovere un pezzo basterà unire la lettera iniziale del suddetto, scrivendola in maiuscolo, alla casa finale che si vuole occupare(E.g. :  **Ca3** per muovere un Cavallo in casa a3, oppure **Dd5** per muovere una Donna in casa d5 ). Per muovere il pedone basterà indicare la casa finale( E.g. : **e4**, **c5**).
...
Inoltre, è stato stimolante lavorare ad un progetto che è rivolto ad un bacino di utenza non strettamente legato all'ambito informatico e reimparare il gioco degli scacchi in chiave algoritmica per scoprire che conoscenze pregresse del gioco non rendono triviale la progettazione di tale software.
...
Un elemento che ha richiesto più tempo del previsto è stata la gestione della dipendenza da Unicode, che è risultato più complessa di quanto aspettato e si è tradotta in più o meno lunghe ricerche di "flag" aggiuntivi per il supporto ad UTF-8.
